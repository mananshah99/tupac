...
def keep_index_in_bounds(array, index):
    if index >= len(array):
        index = len(array)-1

    return int(index)    

def load_data_in_chunks(iter_number, #(0-indexed)
                        chunk_size = 1000, #this is train chunk size, val is just 10 percent
                        train_list = '../lists/roi-train.lst',
                        val_list   = '../lists/roi-val.lst'):
    
    TR_start_idx = iter_number * chunk_size
    TR_end_idx = (iter_number + 1)*chunk_size  # for iter 0, this is 10000, for 1 this is 20000, +1 b/c last index is exclusive

    TS_start_idx = iter_number * (0.1 * chunk_size)
    TS_end_idx   = (iter_number + 1) * (0.1 * chunk_size) 

    train_paths = [i for i in open(train_list, 'r')]
    train_paths = train_paths[keep_index_in_bounds(train_paths, TR_start_idx) : keep_index_in_bounds(train_paths, TR_end_idx)]
    
    val_paths = [i for i in open(val_list, 'r')]
    val_paths = val_paths[keep_index_in_bounds(val_paths, TS_start_idx) : keep_index_in_bounds(val_paths, TS_end_idx)]
    
    train_im_paths = [i.split(" ")[0] for i in train_paths]
    train_labels   = [int(i.split(" ")[1]) for i in train_paths]
    
    val_im_paths   = [i.split(" ")[0] for i in val_paths]
    val_labels     = [int(i.split(" ")[1]) for i in val_paths]

    y_train = np.array(train_labels).astype('int32')
    y_test  = np.array(val_labels).astype('int32')

    X_train  = np.array([imread(i) for i in train_im_paths]) 
    X_train = X_train.reshape(X_train.shape[0], 3, 256, 256).astype('float32')

    X_test  = np.array([imread(i) for i in val_im_paths])
    X_test = X_test.reshape(X_test.shape[0], 3, 256, 256).astype('float32')
...
def load_pickle_data_tupac(train_list = '../lists/roi-train.lst',
                            val_list  = '../lists/roi-val.lst'):

    train_paths = [i for i in open(train_list, 'r')][0:5000]
    val_paths = [i for i in open(val_list, 'r')][0:500]
