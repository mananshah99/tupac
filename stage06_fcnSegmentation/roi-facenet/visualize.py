...
model_def = 'deploy_fullyconv.prototxt'
model_weights = 'models/LEVEL1/cnn10_iter_180000.caffemodel'
...
net.blobs['data'].reshape(100, 3, 100, 100)
...
def get_auc(batch_sz = 100):
    ...
    for i in range(0, int(len(im_lines)/float(batch_sz))):
        ...
        transformed_images = [transformer.preprocess('data', caffe.io.resize_image(caffe.io.load_image(im_name), (100, 100))) for im_name in im_lines_i]
...
sample_image_output = open('../lists/roi-test.lst').readline().strip('\n').split(' ')[1]

image = caffe.io.load_image(sample_image_name)
image = caffe.io.resize_image(image, (100, 100))
...
def show_filters(net):
    net.forward()
    plt.figure()
    filt_min, filt_max = net.blobs['conv42'].data.min(), net.blobs['conv42'].data.max()
    for i in range(3):
        plt.subplot(1,4,i+2)
        plt.title("filter #{} output".format(i))
        plt.tight_layout()
        plt.axis('off')
        plt.imsave('filter #{}'.format(i) + ' .png', net.blobs['conv42'].data[0, i], vmin=filt_min, vmax=filt_max)
