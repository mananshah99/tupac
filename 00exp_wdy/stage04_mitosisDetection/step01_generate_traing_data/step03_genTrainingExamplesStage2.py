...
def gen_patches(img_path, csv_path, msk_image, patch_size = 100, output_directory = "", nn_patch_size = 10, n_patches=10, patch_frac_denominator=4):
...
def get_false_negative(img_path, msk_path, hep_path, csv_path, mitosis_radius=30, num_neg=200, patch_size=100, output_directory = ''):
    ground_truth_points = load_pts(csv_path)
    if 1:
        image = cv2.imread(img_path)
        msk_image = cv2.imread(msk_path, cv2.CV_LOAD_IMAGE_GRAYSCALE)
        ...
        random.seed(1122)
        num_neg_size = np.min((num_neg/2, len(neg_pts)))
...
for line in [l.strip() for l in open('all_image.lst').readlines()]:
    ...
    if 0: # get positive
       gen_patches(img_path, csv_path, msk_path, patch_size=100, n_patches = 10, nn_patch_size = 5, output_directory='training_examples_s2/pos')
    if 1: # get false positive
       get_false_negative(img_path, msk_path, hep_path, csv_path, patch_size=64, num_neg=200, output_directory='training_examples_s2/neg')
    #break
