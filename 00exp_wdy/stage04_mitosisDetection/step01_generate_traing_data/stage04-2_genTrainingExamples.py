...
for line in [l.strip() for l in open('groundtruth.lst').readlines()]:
    ...
    if 0:
        for point in ground_truth_points:
            ...
            # actual mitoses
            gen_patches(img_path, point, msk_image, n_patches = 10, nn_patch_size = 10, output_directory='training_examples/pos')
            #break
    if 1:
        cnts, _ = cv2.findContours(msk_image.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        #msk_image_rgb = cv2.cvtColor(msk_image,cv2.COLOR_GRAY2RGB)
        # other points
        if 1:
            ...
            for cnt in cnts:
                ...
                if notMitosis(tup, ground_truth_points):
                    #cv2.drawContours(msk_image_rgb, [chosen_contour], -1, (0,255,0), 3)
                    #cv2.imwrite('a.png', msk_image_rgb)
                    print "\tNegative => " + "(" + str(tup[0]) + "," + str(tup[1]) + ")"
                    r = gen_patches(img_path, tup, msk_image, output_directory='training_examples/neg', n_patches=1, nn_patch_size = 10)
                    if r:
                        ct = ct + 1
