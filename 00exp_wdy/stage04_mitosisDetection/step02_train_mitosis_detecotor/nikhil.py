...
from lasagne.nonlinearities import softmax, linear, elu

def build_inception_module(name, input_layer, nfilters):
    net = {}
    net['pool'] = PoolLayer(input_layer, pool_size=3, stride=1, pad=1)
    net['pool_proj'] = ConvLayer(
        net['pool'], nfilters[0], 1, nonlinearity=elu, flip_filters=False)

    net['1x1'] = ConvLayer(input_layer, nfilters[1], 1, nonlinearity=elu, flip_filters=False)

    net['3x3_reduce'] = ConvLayer(
        input_layer, nfilters[2], 1, flip_filters=False)
    net['3x3'] = ConvLayer(
        net['3x3_reduce'], nfilters[3], 3, pad=1, nonlinearity=elu, flip_filters=False)
...
def build_GoogLeNet(width, height):
    ...
    net['conv1/7x7_s2'] = ConvLayer(
        net['input'], 64, 7, stride=2, pad=3, nonlinearity=elu, flip_filters=False)
    net['pool1/3x3_s2'] = PoolLayer(
        net['conv1/7x7_s2'], pool_size=3, stride=2)
    net['pool1/norm1'] = LRNLayer(net['pool1/3x3_s2'], alpha=0.00002, k=1)
    net['conv2/3x3_reduce'] = DropoutLayer(ConvLayer(
        net['pool1/norm1'], 64, 1, nonlinearity=elu, flip_filters=False), p = 0.2)
...
def preprocess(im, width, height):
...
def MakeImageArray(data_path, image_path):
    input_images = os.listdir(image_path)
    image_idx=[]
    for input_image in input_images:
        image_idx.append(os.path.splitext(input_image)[0])
    image_idx= np.int32(image_idx)

    df = pd.read_csv(data_path)
    df_withposters = df[(df.movie_id.isin(image_idx))]
    df_clean = df_withposters.dropna(subset=['rating_imdb'])

    images_toload = df_clean['movie_id']

    image_array=[]
    for image_toload in images_toload:
        loading_idx = np.squeeze(np.where(np.asarray(image_idx)==image_toload))
        image_array.append(os.path.join(image_path,input_images[loading_idx]))
    df_clean['poster_path'] = image_array
