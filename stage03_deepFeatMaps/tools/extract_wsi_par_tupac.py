...
class heatmap_generator_par:
    ...
    def get_image(self, wsi, h1_ml, w1_ml):
        h1_ml_2_level0 = h1_ml * np.power(4, self.deep_model_level)
        w1_ml_2_level0 = w1_ml * np.power(4, self.deep_model_level)
        img = wsi.read_region(
    ...
    def has_overlapping_with_tissue_using_mask(self, msk, h1_ml, w1_ml, gap_between_mask_model):
        if msk is None:
            ...
            window_size_kl = int(self.window_size / np.power(4, gap_between_mask_model))
    ...
    def gen_cmd(self, input_list, input_type):
        ...
        cmd.append(
            "python ./tools/extract_wsi_par_tupac.py %s %s %s %s %s %d %d %s --heatmap_level %d --mask_image_level %d --augmentation %d --window_size %d  --step_size %d --batch_size %d --group_size %d"%(
                    self.args.extractor,
                    self.args.conf_file,
                    self.args.feat_name,
            ...
            if os.path.exists(outputName):
                ...
                if 1:
                    ...
                    for (h1_ml, w1_ml), v in tqdm(zip(inds_list, values)):
                        h1_hl = int(h1_ml / np.power(4, self.heatmap_level - self.deep_model_level))
                        w1_hl = int(w1_ml / np.power(4, self.heatmap_level - self.deep_model_level))
                        window_size_hl = int(self.window_size / np.power(4, self.heatmap_level - self.deep_model_level))
                ...
                if 1:
                    ...
                    ## save the heat map ##
                    img_h_kl = img_h_hl / np.power(4, self.mask_image_level - self.heatmap_level)
                    img_w_kl = img_w_hl / np.power(4, self.mask_image_level - self.heatmap_level)
