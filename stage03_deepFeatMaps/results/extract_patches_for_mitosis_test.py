...
def extract_patches(image_number, # A STRING
    ...
    image = imread('/data/dywang/Database/Proliferation/libs/stage03_deepFeatMaps/results/roi-level1-test_07-11-16/thresholded-0.55/TUPAC-TE-' + image_number + '.png')
    wsi = osi.open_slide('/data/dywang/Database/Proliferation/data/TestingData/testing_image_data/TUPAC-TE-' + image_number + '.svs')
...
### STAGE 3: SECOND PASS FOR (SELECTED) REGION PATCH EXTRACTION
    ...
    for i, region in enumerate(final_regions):
        ...
        for centroid in centers:
            ...
            img_name = output_directory + "/TUPAC-TE-" + image_number + "-(" + str(h1_level0) + "," + str(w1_level0) + ").png"
            imsave(img_name, img)
            # print "\t => " + img_name

####

save_directory = "patches-test_08-13-16" #70-14-16 #06-29-16
...
image_numbers = ["{0:0>3}".format(n) for n in range(1, 313)]

from multiprocessing import Pool

for num in image_numbers:
    do_extraction_parallel(num)
